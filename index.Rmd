...
I saw a trend in the data of results in using _Tinder_ and the different ways people used _Tinder_. There were `r people_moblie` who used mobile devices to find people, there were `r people_web` who used the web to meet people, and for Universities `r people_University` people used _Tinder_. Then I checked how many female `r female_respondents`and male `r male_respondents` respondents who used _Tinder_. 
...
```{r chart1, echo = FALSE, warning = FALSE, message = FALSE, code = readLines("Table.R"), fig.width = 10}
kable(gender_df[1:7])
```


The table shows the male and female that have gotten into relationships after using _Tinder_. We see that there are more "no's" than "yes's" present. 
